...
class Params:
    ...
    def __init__(self):
        ...
        self._init_angles = np.negative(np.arange(start=0, stop=15.1, step=2))  # Angles in degrees --> we negate them because the path angle is measured down from the horizon
        self._init_velocities = np.arange(start=0, stop=15_001, step=2_000)    # Possible Initial velocities (m/s)
